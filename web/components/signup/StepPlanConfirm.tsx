...
export function StepPlanConfirm({ data, onChange }: StepPlanConfirmProps) {
  return (
    <div>
      <h2 className="text-2xl font-bold text-[#264653] mb-2">Confirme seu plano</h2>
          ...
          return (
            <button
              ...
              className={cn(
                ...
                selected
                  ? "border-[#f4a261] bg-[#fdf2e9]/50 shadow-lg"
            ...
            >
              <div className="flex items-start justify-between mb-3">
                <div>
                  {planKey === "gold" && (
                    <div className="inline-flex items-center gap-1 bg-[#f4a261] text-white text-xs font-medium px-2 py-0.5 rounded-full mb-2">
                      <Star className="w-3 h-3" /> Mais Popular
                    </div>
                  )}
                  <h3 className="text-lg font-bold text-[#264653]">{plan.name}</h3>
                </div>
                <div
                  className={cn(
                    "w-6 h-6 rounded-full border-2 flex items-center justify-center",
                    selected ? "border-[#f4a261] bg-[#f4a261]" : "border-gray-300"
                  )}
                >
              ...
              <div className="flex items-baseline gap-1 mb-4">
                <span className="text-2xl font-extrabold text-[#264653]">{plan.price}</span>
              ...
              <ul className="space-y-2">
                {plan.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-2 text-sm">
                    <Check className="w-4 h-4 text-[#f4a261] flex-shrink-0 mt-0.5" />
